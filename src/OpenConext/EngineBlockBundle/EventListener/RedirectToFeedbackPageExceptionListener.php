...
public function onKernelException(ExceptionEvent $event)
{
    ...
    $exceptionMap = [
        ...
        EngineBlock_Corto_Module_Bindings_UnsupportedSignatureMethodException::class => [
            ...
            'params' => ['signature-method' => 'signatureMethod']
        ],
        ...
        EngineBlock_Exception_UnknownIdentityProvider::class => [
            ...
            'params' => ['entity-id' => 'entityId', 'destination' => 'destination']
        ],
    ...
    if (isset($config['session'])) {
        $event->getRequest()->getSession()->set($config['session'], $config['message'] ?? $exception->getMessage());
    }

    if (isset($config['params'])) {
        foreach ($config['params'] as $param => $method) {
            $redirectParams[$param] = $exception->{"get$method"}();
        }
    }
