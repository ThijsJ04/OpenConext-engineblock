...
class RedirectToFeedbackPageExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        if (isset($handler['session_key']) && isset($handler['session_value'])) {
            $method = $handler['session_value'];
            $event->getRequest()->getSession()->set($handler['session_key'], $exception->$method());
        }

        // Build redirect parameters
        $redirectParams = [];
        foreach ($handler['params'] as $param) {
            $method = 'get' . $this->camelize($param);
            if (method_exists($exception, $method)) {
                $redirectParams[$param] = $exception->$method();
            }
        }
