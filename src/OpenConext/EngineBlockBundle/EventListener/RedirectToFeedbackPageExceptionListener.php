...
class RedirectToFeedbackPageExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        // Map exception types to their corresponding configuration
        $exceptionMap = [
            ...
            EngineBlock_Corto_Module_Bindings_UnsupportedSignatureMethodException::class => [
                ...
                'params' => function($exception) { return ['signature-method' => $exception->getSignatureMethod()]; }
            ],
            ...
            EngineBlock_Exception_UnknownServiceProvider::class => [
                ...
                'params' => function($exception) { return ['entity-id' => $exception->getEntityId()]; }
            ],
            ...
            UnknownKeyIdException::class => [
                ...
                'params' => function($exception) { return ['keyid' => $exception->getRequestedKeyId()]; }
            ],
            EngineBlock_Corto_Exception_UnknownPreselectedIdp::class => [
                'message' => null, // Use exception message
                'route' => 'authentication_feedback_unknown_preselected_idp',
                'params' => function($exception) { return ['idp-hash' => $exception->getRemoteIdpMd5Hash()]; }
            ],
        ...
        // Special handling for exceptions that need session manipulation
        if ($exception instanceof EngineBlock_Attributes_Manipulator_CustomException) {
            ...
            $config = $exceptionMap[EngineBlock_Attributes_Manipulator_CustomException::class];
            ...
            $config = $exceptionMap[get_class($exception)];
        } elseif ($exception instanceof EntityCanNotBeFoundException) {
            $event->getRequest()->getSession()->set('feedback_custom', $exception->getMessage());
            $config = $exceptionMap[EntityCanNotBeFoundException::class];
        } elseif (isset($exceptionMap[get_class($exception)])) {
            $config = $exceptionMap[get_class($exception)];
        } else {
            return;
        }
        ...
        if (is_callable($config['params'])) {
            $redirectParams = $config['params']($exception);
        } elseif (is_array($config['params'])) {
            $redirectParams = $config['params'];
        }
