...
class RedirectToFeedbackPageExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        // Define exception handlers in a structured way
        $exceptionHandlers = [
            ...
            EngineBlock_Corto_Module_Bindings_UnsupportedSignatureMethodException::class => [
                ...
                'params' => function($e) { return ['signature-method' => $e->getSignatureMethod()]; }
            ],
            ...
            EngineBlock_Exception_UnknownServiceProvider::class => [
                ...
                'params' => function($e) { return ['entity-id' => $e->getEntityId()]; }
            ],
            EngineBlock_Exception_UnknownIdentityProvider::class => [
                ...
                'params' => function($e) { return ['entity-id' => $e->getEntityId(), 'destination' => $e->getDestination()]; }
            ],
        ...
        // If no handler found, return
        if ($handler === null) {
            return;
        }

        // Process the handler
        $message = $handler['message'] ?? $exception->getMessage();
        $redirectToRoute = $handler['route'];
        $redirectParams = [];
        
        // Handle parameters
        if (isset($handler['params']) && is_callable($handler['params'])) {
            $redirectParams = call_user_func($handler['params'], $exception);
        } elseif (isset($handler['params']) && is_array($handler['params'])) {
            $redirectParams = $handler['params'];
        }

        // Handle session data
        if (isset($handler['session'])) {
            $event->getRequest()->getSession()->set($handler['session'], $exception->getMessage());
            if ($handler['session'] === 'feedback_custom' && method_exists($exception, 'getFeedback')) {
