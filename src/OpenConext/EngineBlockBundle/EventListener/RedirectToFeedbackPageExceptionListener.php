...
class RedirectToFeedbackPageExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        $redirectToRoute = $exceptionHandler['route'];
        $redirectParams = $exceptionHandler['params'] ?? [];
    ...
    /**
     * Get exception handler configuration for a given exception.
     *
     * @param \Throwable $exception
     * @return array|null
     */
    private function getExceptionHandler($exception)
    {
        $exceptionHandlers = [
            ...
            EngineBlock_Corto_Module_Bindings_UnsupportedSignatureMethodException::class => [
                ...
                'params_callback' => function($exception) {
        ...
        foreach ($exceptionHandlers as $exceptionClass => $handler) {
            if ($exception instanceof $exceptionClass) {
                // Handle dynamic messages for exceptions that use getMessage()
                if (isset($handler['message']) && $handler['message'] === $exception->getMessage()) {
                    $handler['message'] = $exception->getMessage();
                }
                
                // Handle dynamic parameters using callbacks
                if (isset($handler['params_callback'])) {
                    $handler['params'] = $handler['params_callback']($exception);
                    unset($handler['params_callback']);
                }
