...
class RedirectToFeedbackPageExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        if (!isset($exceptionHandlers[$exceptionClass])) {
            return;
        }

        $handler = $exceptionHandlers[$exceptionClass];
        
        // Handle session data if needed
        if (isset($handler['session_key'])) {
            $sessionValue = $handler['session_value'] === 'feedback' 
                ? $exception->getFeedback() 
                : $exception->getMessage();
            $event->getRequest()->getSession()->set($handler['session_key'], $sessionValue);
        }
        ...
        foreach ($handler['params'] as $paramName) {
            $methodName = 'get' . $this->camelCaseParamName($paramName);
    ...
    /**
     * Convert parameter name to camelCase for method name construction
     *
     * @param string $paramName
     * @return string
     */
    private function camelCaseParamName($paramName)
    {
        return implode('', array_map('ucfirst', explode('-', $paramName)));
    }
}
