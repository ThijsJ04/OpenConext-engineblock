    ...
    public function onKernelException(ExceptionEvent $event)
    {
        ...
        $exceptionHandlers = [
            ...
            EngineBlock_Attributes_Manipulator_CustomException::class => [
                ...
                'callback' => function ($exception, $event) {
        ...
        $handler = $exceptionHandlers[get_class($exception)] ?? null;
        if (!$handler) {
            return;
        }
        ...
        if (isset($handler['callback']) && is_callable($handler['callback'])) {
            $handler['callback']($exception, $event);
        }

        // Determine message
        $message = $handler['message'] ?? $exception->getMessage();

        // Get redirect parameters if callback is defined
        $redirectParams = isset($handler['params_callback']) && is_callable($handler['params_callback'])
            ? $handler['params_callback']($exception)
            : [];
