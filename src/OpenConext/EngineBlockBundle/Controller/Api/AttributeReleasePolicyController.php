...
public function readArpAction(Request $request)
{
    ...
    $body = JsonRequestHelper::decodeContentAsArrayOf($request);
    if (!is_array($body) || !isset($body['entityIds']) || !is_array($body['entityIds']) || empty($body['entityIds'])) {
        throw new BadApiRequestHttpException(
            'Invalid JSON structure: expected root array with non-empty "entityIds" array'
        );
    }
