...
public function startAuthenticationOnBehalfOf(string $requestId, Entity $serviceProvider): void
{
    ...
    $this->authenticationProcedures = $this->authenticationProcedures->add($requestId, $currentAuthenticationProcedure);
}
...
public function completeCurrentProcedure(string $requestId): void
{
    ...
    if ($currentRequest === null) {
        throw new LogicException(
            'The requested authentication procedure with requestId "' . $requestId . '" couldn\'t be found in the session storage in order to complete.'
        );
    }

    if (!$currentRequest->hasBeenAuthenticated()) {
        throw new LogicException(
            'The requested authentication procedure with requestId "' . $requestId . '" has not been authenticated.'
        );
    }
