...
{
    $explanations = [];
    foreach (get_object_vars($setting) as $fieldName => $value) {
        if (str_starts_with($fieldName, 'explanation:')) {
            $langCode = substr($fieldName, 12);
            $explanations[$langCode] = new MultilingualValue($value, $langCode);
        }
    }
    return $explanations;
}
