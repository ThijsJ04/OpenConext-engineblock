...
public function isAllowed($attributeName, $attributeValue)
{
    ...
    foreach ($this->attributeRules[$attributeName] as $rule) {
        ...
        $patternLength = strlen($allowedValue);
        if ($patternLength > 1 && $allowedValue[$patternLength - 1] === self::WILDCARD_CHARACTER) {
            $patternStart = substr($allowedValue, 0, -1);
            if (strncmp($attributeValue, $patternStart, strlen($patternStart)) === 0) {
                return true;
            }
        }
    }
