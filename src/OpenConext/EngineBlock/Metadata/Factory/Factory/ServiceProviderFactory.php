...
public function createStepupEntityFrom(string $keyId): ServiceProviderEntityInterface
{
    ...
    if ($isConfigured && $this->featureConfiguration->isEnabled('eb.stepup.sfo.override_engine_entityid')) {
    ...
    return new ServiceProviderStepup(
        new ServiceProviderEntity(new ServiceProvider($entityId, Mdui::emptyMdui())),
        $this->keyPairFactory->buildFromIdentifier($keyId),
        $this->urlProvider
    );
}
