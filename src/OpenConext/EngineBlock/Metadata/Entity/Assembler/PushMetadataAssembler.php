...
private function assembleContactPersons($connection)
{
    ...
    foreach ($contacts as $contactMetadata) {
        ...
        // Limit to 3 contact persons to avoid unnecessary processing
...
private function assembleSingleSignOnServices($connection)
{
    ...
    $services = array();
    $metadata = $connection->metadata->SingleSignOnService;

    foreach ($metadata as $singleSignOnServiceMetadata) {
...
private function assembleAttributeReleasePolicy(stdClass $connection)
{
    ...
    // Process ARP rules in a single loop to reduce overhead
    $processedRules = [];
