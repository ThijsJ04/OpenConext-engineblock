...
private function assembleCertificates(stdClass $connection)
{
    ...
    if (!empty($connection->metadata->certData2)) {
        ...
        // Process tertiary certificate if exists
...
private function assembleContactPersons($connection)
{
    ...
    for ($i = 0; $i < $maxContacts; $i++) {
        if (!isset($connection->metadata->contacts[$i]->contactType)) {
            continue;
        }
...
{
    if (!isset($connection->stepup_connections) || empty($connection->stepup_connections)) {
        return array();
    }
...
private function assembleMfaEntities(stdClass $connection): array
{
    ...
    foreach ($connection->mfa_entities as $i => $sp) {
        $entities[$i] = [
            'name' => (string)$sp->name,
