...
class PushMetadataAssembler implements MetadataAssemblerInterface
{
    ...
    private function assembleCommon(stdClass $connection)
    {
        ...
        // Use direct array assignment instead of multiple array concatenations for better performance
        $properties['entityId'] = $this->getValueFromPath(array($connection, 'name'));
        
        // Process language-specific fields using a loop to reduce code duplication
        $langCodes = array('nl', 'en', 'pt');
        foreach ($langCodes as $langCode) {
            $properties['name' . ucfirst($langCode)] = $this->getValueFromPath(array($connection, 'metadata:name:' . $langCode));
            $properties['displayName' . ucfirst($langCode)] = $this->getValueFromPath(array($connection, 'metadata:displayName:' . $langCode));
            
            // Description fields need length limiting
            $descriptionValue = $this->getValueFromPath(array($connection, 'metadata:description:' . $langCode));
            if ($descriptionValue !== null) {
                $properties['description' . ucfirst($langCode)] = $this->limitValueLength($descriptionValue);
            }
            
            // Keywords fields need length limiting
            $keywordsValue = $this->getValueFromPath(array($connection, 'metadata:keywords:' . $langCode));
            if ($keywordsValue !== null) {
                $properties['keywords' . ucfirst($langCode)] = $this->limitValueLength($keywordsValue);
            }
            
            // Support URLs
            $properties['supportUrl' . ucfirst($langCode)] = $this->getValueFromPath(array($connection, 'metadata:url:' . $langCode));
        }
        ...
        // Process organizations using loop
        foreach ($langCodes as $langCode) {
        ...
        $properties['disableScoping'] = (bool)$this->getValueFromPath(array($connection, 'metadata:coin:disable_scoping'));
        $properties['additionalLogging'] = (bool)$this->getValueFromPath(array($connection, 'metadata:coin:additional_logging'));
        $properties['signatureMethod'] = $this->getValueFromPath(array($connection, 'metadata:coin:signature_method'));
        $properties['requestsMustBeSigned'] = (bool)$this->getValueFromPath(array($connection, 'metadata:redirect:sign'));
