...
private function assembleCommon(stdClass $connection)
{
    ...
    // Batch process string properties with length limits
    $stringMappings = [
        ['name', 'entityId', false],
        ...
        ['metadata:coin:eula', 'termsOfServiceUrl', false],
        ['metadata:coin:stepup:requireloa', 'stepupRequireLoa', false],
    ];
    ...
    foreach ($boolMappings as [$from, $to]) {
        $properties += $this->setPathFromObjectBool([$connection, $from], $to);
    }
    ...
    $properties += $this->setPathFromObjectString([$connection, 'state'], 'workflowState', false);
    $properties += $this->assembleContactPersons($connection);
    $properties += $this->assembleSingleLogoutServices($connection);

    // Process organization properties
...
private function assembleSingleSignOnServices($connection)
{
    ...
    return empty($services) ? [] : array('singleSignOnServices' => $services);
}
...
private function assembleShibMdScopes($connection)
{
    ...
    return empty($shibMdScopes) ? [] : array('shibMdScopes' => $shibMdScopes);
}

private function assembleStepupConnections(stdClass $connection)
{
    if (!isset($connection->stepup_connections)) {
        return [];
    }
...
private function assembleAssertionConsumerServices(stdClass $connection)
{
    ...
    return ['assertionConsumerServices' => $services];
}
