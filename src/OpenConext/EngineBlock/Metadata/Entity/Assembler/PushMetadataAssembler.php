...
private function assembleSp(stdClass $connection)
{
    ...
    // Batch boolean and string property assignments
    $boolProperties = [
...
private function assembleIdp(stdClass $connection)
{
    ...
    // Batch all property assignments in a single loop
    $propertyMappings = [
        ['metadata:coin:guest_qualifier', 'guestQualifier', false],
        ['metadata:coin:schachomeorganization', 'schacHomeOrganization', false],
        ['metadata:coin:hidden', 'hidden', true],
        ['metadata:coin:defaultRAC', 'defaultRAC', false],
        ['metadata:coin:policy_enforcement_decision_required', 'policyEnforcementDecisionRequired', true],
    ...
    foreach ($propertyMappings as $mapping) {
        if ($mapping[2]) { // bool
            ...
            $properties += $this->setPathFromObjectString([$connection, $mapping[0]], $mapping[1]);
        }
    }
...
private function assembleCommon(stdClass $connection)
{
    ...
    // Batch string property assignments
    $stringMappings = [
        ['name', 'entityId'],
...
private function assembleCertificates(stdClass $connection)
{
    ...
    // Process certificates in a single loop
    for ($i = 1; $i <= 3; $i++) {
        $certData = $connection->metadata->{"certData$i"} ?? null;
        if (empty($certData)) {
            continue;
        }
...
{
    $contactPersons = [];
    $contactMetadata = $connection->metadata->contacts ?? [];

    for ($i = 0; $i < 3; $i++) {
        if (empty($contactMetadata[$i]->contactType)) {
            continue;
        }

        $contact = $contactMetadata[$i];
        $contactPerson = new ContactPerson($contact->contactType);
...
private function assembleAssertionConsumerServices(stdClass $connection)
{
    ...
    foreach ($connection->metadata->AssertionConsumerService as $assertionConsumerServiceMetadata) {
        $location = $assertionConsumerServiceMetadata->Location ?? null;
        $binding = $assertionConsumerServiceMetadata->Binding ?? null;

        if (empty($location) || empty($binding)) {
            continue;
        }

        if (!$this->allowedAcsLocationsValidator->validate($location)) {
        ...
        $services[] = new IndexedService($location, $binding, $index);
        $index += 1;
    }
