...
class DiscoveryAssembler
{
    ...
    public function assembleDiscoveries(stdClass $connection): array
    {
        ...
        foreach ($connection->metadata->discoveries as $discovery) {
            $names = $this->extractLocalizedFieldsEfficient($discovery, 'name', $supportedLanguages);
            
            if (!isset($names['en'])) {
                continue;
            }
            
            $keywords = $this->extractLocalizedFieldsEfficient($discovery, 'keywords', $supportedLanguages);
            $logo = $this->assembleLogoEfficient($discovery);
    ...
    private function extractLocalizedFieldsEfficient(stdClass $discovery, string $fieldPrefix, array $supportedLanguages): array
    {
        $fields = [];
        foreach ($supportedLanguages as $language) {
            $accessor = sprintf('%s_%s', $fieldPrefix, $language);
            if (isset($discovery->$accessor)) {
                $value = trim($discovery->$accessor);
                if ($value !== '') {
    ...
    private function assembleLogoEfficient(stdClass $discovery): ?Logo
    {
