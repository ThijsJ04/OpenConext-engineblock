...
public function assembleDiscoveries(stdClass $connection): array
{
    ...
    foreach ($connection->metadata->discoveries as $discovery) {
        ...
        foreach ($supportedLanguages as $language) {
            $accessor = 'name_' . $language;
            if (isset($discovery->$accessor) && ($trimmed = trim($discovery->$accessor)) !== '') {
                $names[$language] = $trimmed;
            }
        }
        ...
        $keywords = [];
        foreach ($supportedLanguages as $language) {
            $accessor = 'keywords_' . $language;
            if (isset($discovery->$accessor) && ($trimmed = trim($discovery->$accessor)) !== '') {
                $keywords[$language] = $trimmed;
            }
        }

        $logo = null;
        if (isset($discovery->logo_url) && ($url = trim($discovery->logo_url)) !== '') {
            $logo = new Logo($url);
