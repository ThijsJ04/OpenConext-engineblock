...
class DiscoveryAssembler
{
    ...
    public function assembleDiscoveries(stdClass $connection): array
    {
        ...
        foreach ($connection->metadata->discoveries as $discovery) {
            ...
            // Only process if we have an English name
            if (!isset($names['en'])) {
                continue;
            }
    ...
    private function extractLocalizedFields(stdClass $discovery, string $fieldPrefix): array
    {
        ...
        foreach ($this->languageSupportProvider->getSupportedLanguages() as $language) {
            ...
            if (isset($discovery->$accessor) && is_string($discovery->$accessor)) {
                $trimmedValue = trim($discovery->$accessor);
                if ($trimmedValue !== '') {
                    $fields[$language] = $trimmedValue;
                }
            }
        }
    ...
    {
        $logoUrl = $discovery->logo_url ?? null;
        if (!is_string($logoUrl) || trim($logoUrl) === '') {
            return null;
        }

        $logo = new Logo(trim($logoUrl));
