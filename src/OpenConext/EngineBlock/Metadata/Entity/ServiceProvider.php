...
class ServiceProvider extends AbstractRole
{
    ...
    public function getDisplayName(string $preferredLocale = 'en'): string
    {
        ...
        if (empty($spName)) {
            $fallbackProperty = 'name' . ucfirst($preferredLocale);
            $spName = $this->$fallbackProperty ?? '';
        }

        // Fallback to English if preferred locale didn't yield results and it's not English
