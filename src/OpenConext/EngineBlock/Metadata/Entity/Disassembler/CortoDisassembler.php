...
private function translateOrganization(AbstractRole $entity, array $cortoEntity)
{
    ...
    foreach ($languages as $lang => $orgProperty) {
        if ($entity->$orgProperty) {
            foreach ($fields as $field) {
                if (isset($entity->$orgProperty->$field)) {
                    $this->mapMultilang($entity->$orgProperty->$field, $cortoEntity, 'Organization', ucfirst($field), $lang);
                }
            }
        }
    }
