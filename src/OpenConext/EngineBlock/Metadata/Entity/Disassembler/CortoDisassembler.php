...
public function translateIdentityProvider(IdentityProvider $entity)
{
    ...
    $consentSettings = $entity->getConsentSettings();
    $spEntityIdsWithoutConsent = $consentSettings->getSpEntityIdsWithoutConsent();
    if (!empty($spEntityIdsWithoutConsent)) {
        $cortoEntity['SpsWithoutConsent'] = $spEntityIdsWithoutConsent;
    } else {
    ...
    foreach ($organizations as $lang => $organization) {
        ...
        foreach ($fields as $field => $key) {
            if (!empty($organization->$field)) {
                $this->mapMultilang($organization->$field, $cortoEntity, 'Organization', $key, $lang);
            }
        }
    }
