...
final class AdditionalInfo
{
    ...
    public static function createFromException(EngineBlock_Exception $exception)
    {
        ...
        // Map severity using a lookup array for better performance
        static $severityMap = [
        ...
        // Build details more efficiently
        $details = [];
        ...
        // Process exception chain more efficiently
        $current = $exception;
        do {
            $details[] = get_class($current) . ': ' . $current->getMessage();
            $details[] = $current->getTraceAsString();
            $current = $current->getPrevious();
        } while ($current !== null);

        $info->details = implode(PHP_EOL, $details);
