...
private function getOrganizationDisplayNameFields(): array
{
    ...
    // Process all languages in order
    foreach (['en', 'nl', 'pt'] as $lang) {
        if (!empty($organizations[$lang]->displayName)) {
            $fields['organization_display_name'][$lang] = $organizations[$lang]->displayName;
        } elseif (!empty($organizations[$lang]->name)) {
            $fields['organization_display_name'][$lang] = $organizations[$lang]->name;
        } else {
