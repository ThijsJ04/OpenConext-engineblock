...
public function jsonSerialize(): array
{
    ...
    $supportEmail = reset($supportContacts) ? reset($supportContacts)->emailAddress : null;
...
private function getDisplayNameFields(): array
{
    ...
    foreach ($languages as $lang) {
        ...
        $fields['display_name'][$lang] = $displayName ?? $this->serviceProvider->{"name$lang"} ?? $this->serviceProvider->entityId;
    }
...
private function getOrganizationDisplayNameFields(): array
{
    ...
    // Process English first to ensure it's available for fallback
    $fields['organization_display_name']['en'] =
        (!empty($organizations['en']->displayName)) ?
        $organizations['en']->displayName :
        (!empty($organizations['en']->name) ? $organizations['en']->name : 'unknown');

    // Process other languages with proper fallback
    foreach (['nl', 'pt'] as $lang) {
        $fields['organization_display_name'][$lang] =
            (!empty($organizations[$lang]->displayName)) ?
            $organizations[$lang]->displayName :
            (!empty($organizations[$lang]->name) ? $organizations[$lang]->name : $fields['organization_display_name']['en']);
    }
