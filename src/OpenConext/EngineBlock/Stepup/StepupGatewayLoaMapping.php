...
public function __construct(array $loaMapping, string $gatewayLoa1, LoaRepository $loaRepository)
{
    ...
    foreach ($loaMapping as $mapping) {
        ...
        $gatewayLoas[] = $mapping['gateway'];
        $engineBlockLoas[] = $mapping['engineblock'];
    }

    $uniqueGatewayLoas = array_unique($gatewayLoas);
    $uniqueEngineBlockLoas = array_unique($engineBlockLoas);

    if (count($uniqueGatewayLoas) !== count($gatewayLoas)) {
    ...
    if (count($uniqueEngineBlockLoas) !== count($engineBlockLoas)) {
        throw new \OpenConext\EngineBlock\Exception\InvalidArgumentException(
