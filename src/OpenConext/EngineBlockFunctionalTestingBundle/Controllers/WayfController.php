...
public function wayfAction(Request $request)
{
    ...
    $backLink = $request->get('backLink', false);
    $displayUnconnectedIdpsWayf = $request->get('displayUnconnectedIdpsWayf', false);
    $addDiscoveries = $request->get('addDiscoveries', true);
    $rememberChoiceFeature = $request->get('rememberChoiceFeature', false);
    ...
    $idpList = $randomIdps === 0
        ? TestEntitySeeder::buildIdps($connectedIdps, $unconnectedIdps, $currentLocale, $request->get('defaultIdpEntityId', null), $addDiscoveries)
        : TestEntitySeeder::buildRandomIdps($randomIdps, $currentLocale, $request->get('defaultIdpEntityId', null));
