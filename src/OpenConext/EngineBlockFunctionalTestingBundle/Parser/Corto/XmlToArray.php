...
public static function xml2array($xml)
{
    ...
    $result = self::xml2arrayRecursive($values);
    xml_parser_free($parser);
    return $result[0];
}
...
protected static function xml2arrayRecursive(&$elements, $level = 1, $namespaceMapping = [])
{
    ...
    for ($i = self::$counter; $i < $count; $i++) {
        ...
        $complete = [self::TAG_NAME_PFX => self::mapNamespacesToSaml($tagName)];
        if ($hashedAttributes) {
...
public static function formatXml($xml)
{
    ...
    foreach ($lines as $line) {
        ...
        $result .= str_pad($line, strlen($line) + $pad, ' ', STR_PAD_LEFT) . "\n";

        // opening tag - increase pad for next line
        if (preg_match('/^<\w[^>]*[^\/]>.*$/', $line)) {
            $pad++;
        }
    }
