...
private static function transformIdpsForWayf(array $idpEntityIds, $currentLocale)
{
    ...
    foreach ($identityProviders as $identityProvider) {
        ...
        $wayfIdps[] = $baseWayfIdp;

        foreach ($identityProvider->getDiscoveries() as $discovery) {
            $discoveryWayfIdp = $baseWayfIdp;
            $discoveryWayfIdp['Name'] = $discovery->getName($currentLocale);
            $discoveryWayfIdp['Logo'] = $discovery->getLogo() ? $discovery->getLogo()->url : '/images/placeholder.png';
            $discoveryWayfIdp['Keywords'] = $discovery->getKeywords('en');
            $discoveryWayfIdp['DiscoveryHash'] = $discoveryService->hash($discovery);
            $wayfIdps[] = $discoveryWayfIdp;
        }
    }
