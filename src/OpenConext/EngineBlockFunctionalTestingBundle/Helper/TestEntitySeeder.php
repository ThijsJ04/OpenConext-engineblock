...
public static function buildIdps($numberOfIdps, $numberOfUnconnectedIdps, $locale, $defaultIdpEntityId, bool $addDiscoveries)
{
    ...
    for ($i = 1; $i <= $connectedCount; $i++) {
        ...
        $discoveries = $addDiscoveries && $i === 1 ? $discoveryTemplate : [];
    ...
    for ($i = 1; $i <= $numberOfUnconnectedIdps; $i++) {
        ...
        $isDefaultIdp = $defaultIdpEntityId === $entityId;
        $discoveries = $addDiscoveries && $i === 1 ? $disconnectedDiscoveryTemplate : [];
...
public static function buildRandomIdps($numberOfIdps, $locale, $defaultIdpEntityId)
{
    ...
    $nameCount = count($idpNames);

    for ($i = 1; $i <= $numberOfIdps; $i++) {
        $connected = random_int(0, 1) === 1;
        $entityId = $connected
            ? "https://example.com/entityId/$i"
            : "https://unconnected.example.com/entityId/$i";

        $name = $i <= $nameCount
            ? sprintf("%s %d %s", $idpNames[$randomIdpNames[$i - 1]], $i, $locale)
...
private static function transformIdpsForWayf(array $idpEntityIds, $currentLocale)
{
    ...
    $wayfIdps = array();
    $placeholderLogo = '/images/placeholder.png';

    foreach ($identityProviders as $identityProvider) {
        $name = 'name' . ucfirst($currentLocale);
        $idpData = $idpEntityIds[$identityProvider->entityId];
        $isDefaultIdp = $idpData['isDefaultIdp'];
        $enabledInWayf = $identityProvider->enabledInWayf ? '1' : '0';
        $logoUrl = $identityProvider->logo ? $identityProvider->logo->url : $placeholderLogo;
        $idpHash = md5($identityProvider->entityId);

        $wayfIdps[] = [
            'Name' => $identityProvider->$name,
            'Logo' => $logoUrl,
            'Keywords' => $identityProvider->keywordsEn,
            'Access' => $enabledInWayf,
            'ID' => $idpHash,
        ...
        foreach ($identityProvider->getDiscoveries() as $discovery) {
            $wayfIdps[] = [
                ...
                'Logo' => $discovery->getLogo() ? $discovery->getLogo()->url : $placeholderLogo,
...
{
    $idps = [];
    $placeholderLogo = new Logo('/images/logo.png');
    $placeholderKeywords = 'Awesome IdP, Another keyword, Example';

    foreach ($idpEntityIds as $idpEntityId => $idpData) {
        $idp = new IdentityProvider($idpEntityId);
        $idp->getMdui()->setLogo($placeholderLogo);
        $idp->nameEn = $idpData['name'];
        $idp->nameNl = $idpData['name'];
        $idp->namePt = $idpData['name'];
        $idp->keywordsEn = $placeholderKeywords;
...
public static function buildIdP(?string $idpName)
{
    ...
    foreach ($suffixes as $suffix) {
        $identityProvider->{"name$suffix"} = "$idpName $suffix";
        $identityProvider->{"displayName$suffix"} = "$idpName $suffix";
    }
