...
public static function buildIdps($numberOfIdps, $numberOfUnconnectedIdps, $locale, $defaultIdpEntityId, bool $addDiscoveries)
{
    ...
    // Pre-define discoveries to avoid repeated creation
    $defaultDiscoveries = $addDiscoveries ? [
        Discovery::create(
    ...
    for ($i = 1; $i <= $connectedCount; $i++) {
        ...
        $discoveries = $i === 1 ? $defaultDiscoveries : [];
    ...
    foreach ($identityProviders as $identityProvider) {
        ...
        foreach ($identityProvider->getDiscoveries() as $discovery) {
            $wayfIdps[] = array(
                ...
                'Logo' => $discovery->getLogo() ? $discovery->getLogo()->url : $logoUrl,
