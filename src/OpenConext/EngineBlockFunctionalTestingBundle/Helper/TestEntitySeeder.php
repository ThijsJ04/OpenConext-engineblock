...
public static function buildIdps($numberOfIdps, $numberOfUnconnectedIdps, $locale, $defaultIdpEntityId, bool $addDiscoveries)
{
    ...
    for ($i = 1; $i <= $connectedCount; $i++) {
        ...
        if ($addDiscoveries && $i === 1) {
            foreach ($discoveryData as $discovery) {
                $discoveries[] = Discovery::create($discovery[0], $discovery[1], $discovery[2]);
            }
        }
    ...
    if ($numberOfUnconnectedIdps > 0) {
        ...
        for ($i = 1; $i <= $numberOfUnconnectedIdps; $i++) {
            ...
            $discoveries = [];
            if ($addDiscoveries && $i === 1) {
                foreach ($disconnectedDiscoveryData as $discovery) {
                    $discoveries[] = Discovery::create($discovery[0], $discovery[1], $discovery[2]);
                }
            }
...
public static function buildRandomIdps($numberOfIdps, $locale, $defaultIdpEntityId)
{
    ...
    for ($i = 1; $i <= $numberOfIdps; $i++) {
        ...
        $name = $i < 25 ? sprintf("%s %d %s", $idpNames[$randomIdpNames[$i - 1]], $i, $locale) : sprintf("%s IdP %d %s", $connected ? 'Connected' : 'Disconnected', $i, $locale);
        $isDefaultIdp = $defaultIdpEntityId === $entityId;
