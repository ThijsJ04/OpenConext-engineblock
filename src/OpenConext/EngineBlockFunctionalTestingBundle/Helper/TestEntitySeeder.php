...
public static function buildRandomIdps($numberOfIdps, $locale, $defaultIdpEntityId)
{
    ...
    $idps = [];
    $randomIdpNames = array_rand($idpNames, min($numberOfIdps, count($idpNames)));
    $useRandomNames = $numberOfIdps <= count($idpNames);
    ...
    for ($i = 1; $i <= $numberOfIdps; $i++) {
        ...
        $name = $useRandomNames && $i <= count($idpNames)
...
private static function transformIdpsForWayf(array $idpEntityIds, $currentLocale)
{
    ...
    foreach ($identityProviders as $identityProvider) {
        ...
        $enabledInWayf = $identityProvider->enabledInWayf;
        $access = $enabledInWayf ? '1' : '0';
        $entityId = $identityProvider->entityId;
        $idpHash = md5($entityId);
