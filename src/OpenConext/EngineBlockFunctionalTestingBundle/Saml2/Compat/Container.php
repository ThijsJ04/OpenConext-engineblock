...
public function postRedirect($url, $data = []): void
{
    ...
    foreach ($data as $name => $value) {
        $formData .= sprintf('            <input name="%s" type="text" value="%s" />' . PHP_EOL,
    ...
    if (isset($data['SAMLRequest'])) {
        ...
        $authnRequestXml = htmlentities(self::formatXml($requestXml), ENT_QUOTES, 'utf-8');
    }

    $responseDebug = '';
    if (isset($data['SAMLResponse'])) {
        $responseXml = base64_decode($data['SAMLResponse']);
        $responseDebug = '<pre id="responseDebug">' . htmlentities(self::formatXml($responseXml), ENT_QUOTES, 'utf-8') . '</pre>';
    }
