...
public function iShouldSeeARTCode($artCode)
{
    ...
    if (!$result || $result->getText() !== $artCode) {
        throw new RuntimeException(
            sprintf('Expected Error Code "%s" did not match the Error Code on the page "%s"', $artCode, $result ? $result->getText() : 'null')
        );
    }
}
...
public function iExploitTheXMLSignatureBypass()
{
    ...
    // Remove response signature if present
    $responseSignatures = $xpath->query("/saml_protocol:Response/ds:Signature");
    foreach ($responseSignatures as $node) {
    ...
    $attributeValueNode->textContent = "ADMIN!";
