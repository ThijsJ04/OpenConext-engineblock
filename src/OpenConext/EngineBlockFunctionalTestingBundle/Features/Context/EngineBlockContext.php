...
public function aLangCookieShouldBeSetWithValue($locale)
{
    ...
    if ($cookie === null || $cookie !== $locale) {
        throw new ExpectationException(
            sprintf('The lang cookie should contain "%s", but contains "%s"', $locale, $cookie ?? 'null'),
...
public function iShouldSeeARTCode($artCode)
{
    ...
    // Use a single XPath query to directly find and validate the ART code
    $xpath = '//span[text()="EC:"]/following-sibling::span[1][text()="' . $artCode . '"]';
    $result = $page->find('xpath', $xpath);

    if (!$result) {
        throw new RuntimeException(
            sprintf('Expected Error Code "%s" was not found on the page', $artCode)
        );
    }
}
...
public function iExploitTheXMLSignatureBypass()
{
    ...
    // 1. Clone original assertion and wrap it
    ...
    // 2. Remove signatures from both assertions
