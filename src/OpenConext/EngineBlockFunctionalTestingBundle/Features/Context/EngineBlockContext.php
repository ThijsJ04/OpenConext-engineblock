...
public function iSelectOnTheWAYF($idpName)
{
    ...
    $button = $this->getMinkContext()->getSession()->getPage()->find('css', $selector);
...
public function theReceivedAuthnRequestShouldNotMatchXpath($xpath)
{
    ...
    try {
        ...
        if (false === strpos($e->getMessage(), 'did not result in at least one match')) {
            throw new ExpectationException(
                'Unexpected match on the xpath that should NOT match the AuthnRequest xml',
                $session,
                $e
            );
        }
...
public function iExploitTheXMLSignatureBypass()
{
    ...
    // Update assertion ID and compute new digest
    $assertion->setAttribute('ID', 'attack');
    $digestValue = base64_encode(hash('sha256', $assertion->C14N(true, false), true));

    // Update SignedInfo with new digest and reference
